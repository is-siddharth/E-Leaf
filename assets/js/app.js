...
function brandMark(){
  return `<span class="app-brand-mark" aria-hidden="true"><svg viewBox="0 0 64 64"><path d="M32 6C46 11 54 22 54 34C54 48 44 58 32 58C20 58 10 48 10 34C10 22 18 11 32 6Z"/><path d="M32 10v44M32 24c-4 2-8 5-8 5M32 34c-5 2-9 6-9 6M32 44c-5 2-8 5-8 5" class="brand-leaf-vein"/></svg></span>`;
}
function contextOrientation(){
  ...
  return `<section class="context-orientation" aria-label="Current E-Leaf context">
    ...
    <button type="button" class="orientation-space-switch" id="changeContextMobile" aria-label="Change E-Leaf space"><span>${icon(global?'globe':'building')}</span><span>Change space</span></button>
  </section>`;
}
...
function renderApp(){
  ...
  $('#appView').innerHTML=`<div class="app-shell ${worldClass} ${roleClass}">
    ...
    <main class="app-body">${contextOrientation()}<nav class="app-nav" id="appNav"></nav><div id="appContent"></div></main><div id="growthDock"></div>
  </div>`;
  show($('#appView'));
...
function renderGrowthDock(animate=false){
  ...
  const fillY=204-(Math.max(0,Math.min(100,pct))*1.87);
...
function renderGlobalHome(name){
  ...
  <aside><div class="app-card next-step-card"><div class="eyebrow">A GOOD NEXT STEP</div><h3>${tree?'Help a learner outside your classroom.':'Learn something, then make it useful to someone else.'}</h3><p>${tree?'Answer a question where your approved teaching area fits.':'Your journey is not only about finishing lessons. Contribution is part of learning here.'}</p><button class="btn" onclick="routeApp('${tree?'community':'journey'}')">${tree?'Open community':'See your journey'}</button></div></aside></div>
...
function renderExplore(query=''){
  ...
  $('#appContent').innerHTML=`<div class="welcome"><div><div class="eyebrow">${inst?'COLLEGE':'GLOBAL'} · EXPLORE</div><h1>${inst?'Explore what your college can teach you.':'Explore without a fixed curriculum.'}</h1><p>${inst?'Follow subjects and people inside your institution.':'Search by what you want to learn, then choose a subject, Tree, class, note, or question.'}</p></div></div><div class="explore-search"><span>${icon('search')}</span><input id="exploreSearch" value="${escapeHtml(query)}" aria-label="Search Explore" placeholder="Search subjects, Trees, courses, classes…"><button class="btn ${context.role==='tree'?'btn-tree':inst?'btn-leaf':'btn-global'}" id="exploreSearchButton">Search</button></div><div class="explore-results-head"><div><div class="eyebrow">${q?`RESULTS FOR “${escapeHtml(query)}”`:'START EXPLORING'}</div><h2>${q?`${courses.length+trees.length} demo matches`:'Subjects and people'}</h2></div></div><div class="course-grid">${courses.map(c=>`<button class="course" style="text-align:left" onclick="routeApp('learn')"><span class="course-tag">${c.subject}</span><h3>${c.title}</h3><p>${c.detail}</p><div class="course-meta"><span>${c.meta.split(' · ')[0]}</span><span>${inst?'Your college':'Global'}</span></div></button>`).join('')||'<div class="empty-state"><strong>Nothing matched yet.</strong><span>Try a subject such as physics, programming, or philosophy.</span></div>'}</div><div class="explore-people"><div class="eyebrow">TREES</div><h2>Learn from people, not just content.</h2><div class="people-mini-grid">${trees.map(([name,subject,desc])=>`<article class="person-mini"><div class="person-avatar avatar-sage">${initials(name)}</div><div><span>${subject} · Global</span><h3>${name}</h3><p>${desc}</p><button class="btn">View Tree</button></div></article>`).join('')||'<div class="empty-state"><strong>No Tree matches.</strong><span>Try searching for a subject.</span></div>'}</div></div>`;
...
function renderJourney(){
  ...
  $('#appContent').innerHTML=`<div class="welcome"><div><div class="eyebrow">GLOBAL · JOURNEY</div><h1>${tree?'Keep learning. Keep teaching.':'See how learning can become contribution.'}</h1><p>${tree?'You have crossed the demo threshold. The Tree space adds teaching without taking away your learner identity.':'For this presentation demo, three simple actions show the transition from Leaf to Tree. Production qualification will be much more rigorous.'}</p></div><button class="btn ${tree?'btn-tree':'btn-global'}" onclick="${tree?"routeApp('teach')":"routeApp('learn')"}">${tree?'Open teaching':'Start learning'}</button></div>
  <div class="journey-layout"><section class="journey-card"><div class="journey-header"><div><div class="eyebrow">DEMO GROWTH PATH</div><h2>Learn → Share → Help → Teach</h2></div><span class="journey-count">${tree?3:done} / 3</span></div><div class="journey-steps">${steps.map(([key,title,desc],i)=>`<div class="journey-step ${growth.actions.includes(key)?'done':''} ${!tree&&i===done?'current':''}"><span class="journey-step-mark">${growth.actions.includes(key)?'✓':i+1}</span><div><strong>${title}</strong><p>${desc}</p></div>${!tree&&!growth.actions.includes(key)?`<button class="btn btn-global" data-journey-action="${key}">Do this</button>`:'<span class="journey-status">'+(growth.actions.includes(key)?'Complete':'Next')+'</span>'}</div>`).join('')}</div>${tree?'<div class="journey-unlocked"><div class="trust-symbol">'+icon('tree')+'</div><div><div class="eyebrow">TREE UNLOCKED · DEMO</div><strong>Teaching capability is now visible.</strong><p>This prototype intentionally uses three simple actions so the transition can be demonstrated in a presentation.</p></div></div>':''}</section><aside><div class="tree-panel ${tree?'is-tree':''}"><div class="tree-symbol">${icon(tree?'tree':'leaf')}</div><div class="eyebrow">${tree?'GLOBAL TREE CAPABILITY':'GLOBAL LEAF'}</div><h3>${tree?'Approved area: Physics · Demo':'Your growth stays yours.'}</h3><p>${tree?'In production, this would be backed by a reviewed qualification and scoped teaching areas.':'The same E-Leaf identity remains a learner. The demo only reveals the teaching transition after three contributions.'}</p><button class="btn" onclick="${tree?"routeApp('teach')":"routeApp('community')"}">${tree?'View teaching space':'Help someone'}</button></div></aside></div>`;
  $$('#appContent [data-journey-action]').forEach(b=>b.onclick=()=>recordGrowthAction(b.dataset.journeyAction));
}
function renderTeach(){
  $('#appContent').innerHTML=`<div class="welcome"><div><div class="eyebrow">Teach</div><h1>Help people learn well.</h1><p>Your teaching space should be about learners, sessions, and useful explanations, not vanity metrics.</p></div><button class="btn btn-primary">Plan a session</button></div><div class="app-grid"><section><div class="app-card"><div class="eyebrow">Next session</div><h2 style="margin-top:9px">Thinking clearly about probability</h2><p>Prepare a small learning session for people exploring evidence and uncertainty.</p><button class="btn btn-primary">Open session</button></div><div class="card-row"><div class="app-card small-action"><span class="eyebrow">Learners</span><strong>See your learners</strong><span>Questions, attendance, and follow-up.</span></div><div class="app-card small-action"><span class="eyebrow">Create</span><strong>Make a learning experience</strong><span>Class, lesson, or useful resource.</span></div><div class="app-card small-action"><span class="eyebrow">Help</span><strong>Answer a question</strong><span>Contribute where your subject knowledge fits.</span></div></div></section><aside><div class="tree-panel"><div class="tree-symbol">${icon('tree')}</div><div class="eyebrow">CAPABILITY</div><h3>Physics · Demo teaching area</h3><p>This presentation build shows a scoped Tree capability. Production teaching areas will be granted through qualification and review.</p></div></aside></div>`;
}
